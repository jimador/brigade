...
back to `"${CLAUDE_PLUGIN_ROOT}/scripts/brigade-config" get tier`) is `three-star` and this is an end-of-dish retro,
standard otherwise (mid-dish checkpoints are always standard).

Standard: dispatch `brigade-analyst` exactly per `RETRO.md` (next to the brigade
SKILL): pass the dish dir, `.brigade/LEARNINGS.md`, and the output path
...
efficiency block from `"${CLAUDE_PLUGIN_ROOT}/scripts/brigade-status"`, and the live heuristic set (the KB search command
